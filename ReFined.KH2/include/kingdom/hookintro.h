...
namespace Tz
{
	...
	{
	protected:
		static inline vector<char*> INTRO_OFFSETS = vector<char*>
		{
			FindSignature<char*>("\x48\x89\x5C\x24\x18\x55\x56\x57\x41\x54\x41\x55\x41\x56\x41\x57\x48\x83\xEC\x50\x48\x8B\x05\x00\x00\x00\x00", "xxxxxxxxxxxxxxxxxxxxxxx????"),
			FindSignature<char*>("\x48\x89\x5C\x24\x20\x55\x56\x57\x41\x54\x41\x55\x41\x56\x41\x57\x48\x83\xEC\x50\x4C\x8B\x3D\x00\x00\x00\x00", "xxxxxxxxxxxxxxxxxxxxxxx????"),
			FindSignature<char*>("\x40\x53\x48\x83\xEC\x20\x48\x8B\x0D\x00\x00\x00\x00\x48\x81\xC1\x40\x04\x00\x00\xE8\x00\x00\x00\x00", "xxxxxxxxx????xxxxxxxx????"),
			FindSignature<char*>("\x48\x83\xEC\x38\x48\x8B\x0D\x00\x00\x00\x00\x48\x89\x5C\x24\x40\x48\x85\xC9\x74\x27\xE8\x00\x00\x00\x00", "xxxxxxx????xxxxxxxxxxx????"),
			FindSignature<char*>("\x48\x89\x5C\x24\x10\x57\x48\x83\xEC\x40\x48\x8B\x05\x00\x00\x00\x00\x80\x78\x0C\x00\x74\x18\xE8\x00\x00\x00\x00", "xxxxxxxxxxxxx????xxxxxxx????"),
			FindSignature<char*>("\x48\x89\x5C\x24\x20\x56\x57\x41\x56\x48\x83\xEC\x30\xE8\x00\x00\x00\x00\x48\x8B\xC8\xE8\x00\x00\x00\x00", "xxxxxxxxxxxxxx????xxxx????"),
			FindSignature<char*>("\x48\x83\xEC\x28\xE8\x00\x00\x00\x00\x8B\x15\x00\x00\x00\x00\x48\x8B\xC8\xE8\x00\x00\x00\x00", "xxxxx????xx????xxxx????")
		};

		static inline uint16_t FIRST_CONFIG = 0x0408;
		static inline uint16_t SECOND_CONFIG = 0x0000;
	...
	public:
		...
		static inline bool INTRO_APPLIED;
		static inline bool ENFORCE_INTRO;
			    
		static inline map<uint32_t**, Tz::HookIntro::Entry> IntroSeeks;

		static inline vector<Entry> Entries = vector<Tz::HookIntro::Entry>
		{
		...
		static void Submit()
		{
			...
			uint32_t _menuOffset = _introMemory - moduleInfo.moduleStart;
			...
			auto _fetchInit = FindSignature<char*>("\x48\x89\x5C\x24\x20\x56\x57\x41\x56\x48\x83\xEC\x30\xE8", "xxxxxxxxxxxxxx");
		...
		static void Add(int Index, Tz::HookIntro::Entry Input)
		{
			if (Index == UINT32_MAX)
