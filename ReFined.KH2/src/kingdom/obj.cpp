...
float YS::OBJ::get_top_matrix(char* obj, char* matrix)
{
	if (obj)
	{
		...
		YS::OBJ::get_bone_matrix(obj, *reinterpret_cast<uint16_t*>(_fetchElem + 0x06), matrix);
		return static_cast<float>(*reinterpret_cast<uint16_t*>(_fetchElem + 0x08));
	}
}